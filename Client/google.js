...
// Affichage des positions des joueurs visibles par le client
function afficheLoc(data)
{
    ...
    for	(index = 0; index < data.length; index++) {
        ...
        for	(index_equipe = 0; index_equipe < equipe.length; index_equipe++) {
            ...
            var longitude = parseFloat(coordonnees[0]); // Longitude
            var latitude = parseFloat(coordonnees[1]); // Latitude
            ...
            if (point.id != id){
                ...
                var distance = google.maps.geometry.spherical.computeDistanceBetween( latlngClient, latlng);
                
                // Affichage des marqueurs que sous certaines conditions
                if ((distance < rayon || index == 0) && point.capa > 0 && capa > 0 )
                {
...
function maPosition(position) {
    ...
    var fogWar = {
      ...
      fillOpacity: 0.5,
      map: map,
      paths: [outerbounds,drawCircle(latlng,rayon,-1)]
    };
    ...
    */
    ...
    //Ajout du marqueur à la carte
    marker.setMap(map);
    requeteAjaxLocalisation("id="+id+"&loc=("+position.coords.longitude+","+position.coords.latitude+")&partie="+partie+"&equipe="+equipe);    
        ...
        { 
			if(ajax.readyState == 4 && ajax.status == 200)  // Si le .php a bien renvoyé des données
			{
                ...
                if (capa <= 0 ){
                    ...
                    derniereAction.innerHTML = "";
                    parametrageDerniereAction.innerHTML = "Vous avez été tué";
                    etat = 'recup';
                ...
                if (id == 1)
                {
                    ...
                    btnValider.addEventListener("click", function(){
                		...
                		else if (init==1)
                		{
		                    ...
		                    nombreBalise = document.createElement("SELECT");
							for(var i=1; i<=6; i++){
                                var option = document.createElement("OPTION");
                                option.text = i;
                                option.value = i;
                                nombreBalise.options.add(option);     
                            }
                		...
                		{	
			                    ...
			                    initialisation.removeChild(initialisation.getElementsByTagName("SELECT")[0]);
			                    for (var j=1; j<=2; j++)
			                    {
			                    	batterie = new Object;
...
function afficherQG(listeObjetQGBatterie)
{
    ...
    for (index_listeObjetQGBatterie = 0; index_listeObjetQGBatterie < listeObjetQGBatterie.length; index_listeObjetQGBatterie++) {
        ...
        if (point.qg_bat)
        {
            if (equipe == point.equipe)
            {
                QGBar.value = point.capa;
            }
            if (point.capa <= 0 )
            {
                document.getElementById("boutons").style.visibility = "hidden";
                ...
                if (equipe == point.equipe)
                {
                    ...
                    parametrageDerniereAction.innerHTML = "DEFAITE";
                }
                else 
                {
                    document.body.style.background="#00FF00";
                    parametrageDerniereAction.innerHTML = "VICTOIRE";
                }
...
window.addEventListener('load',function (e){
    ...
    QGBar.max = capaMax*10;
    QGBar.value = capaMax*10;
